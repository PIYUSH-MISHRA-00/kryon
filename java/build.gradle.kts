...
signing {
    // Signing is required by Maven Central and irrelevant everywhere else, so
    // it activates only when a key is actually supplied. A build that silently
    // produced unsigned artifacts and called them releasable would be worse
    // than one that fails.
